...
import ZOSAPI.*;
    ...
    if ~isempty(HammerOpt)
        ...
        HammerOpt.Run(HammerOptimTimeInSeconds);
        for k = 1:15
            pause(1.0);
            fprintf(TheMFE.GetOperandAt(4).Value);
        end
        HammperOpt.Cancel();
...
NET.addAssembly('C:\Program Files\Zemax OpticStudio 17.5\ZOS-API\Libraries\ZOSAPI_NetHelper.dll');
